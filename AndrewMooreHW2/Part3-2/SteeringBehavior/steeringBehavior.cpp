...
SteeringData Wander::getSteering() {
    float wander_orientation = (float)std::rand()/RAND_MAX * wanderRate;
    if ((float)std::rand()/RAND_MAX > 0.5) {
        wander_orientation *= -1;
    }
...
SteeringData Flock::getSteering() {
    ...
    SteeringData wan = wander.getSteering();

    // Blending
    SteeringData result = sep + velM * 5 + arr * 2 + ali;
