...
DecisionTreeNode* NextToWallDecision::getBranch() {
    ...
    std::vector<WorldVertex*> aroundVerts = world->getWorldVerticesAround(character);
...
RandomDecision::RandomDecision(DecisionDecisionTreeNode* trueNode, DecisionTreeNode* falseNode) : Decision(trueNode, falseNode) {}

DecisionTreeNode* RandomDecision::getBranch() {
    std::srand(std::time(nullptr));
    float percent = (float)std::rand()/RAND_MAX;
    return percent > 0.99 ? trueNode : falseNode;
}
