...
std::vector<WorldVertex*> WorldGraph::getWorldVerticesAround(WorldVertex* middle) {
    std::vector<WorldVertex*> list;
    int i = middle->getID();
    if (i >= columns) {
        list.push_back(world_vertices[i - columns]);
    }
    if (i % columns > 0) {
        list.push_back(world_vertices[i - 1]);
    }
